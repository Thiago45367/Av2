...
public class TelaLogin extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        tableModel.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "id", "paciente", "medico", "data", "hora", "observação"
            }
        ));
    ...
    public void consultaslistadas() {
        ...
        ArrayList<Consulta> listar = controller.listar();
        
        for (Consulta consulta : listar) {
            
            int id_Paciente = consulta.getId_paciente();
            Paciente paciente = pacienteDao.buscarPorId(id_Paciente);
            
            int id_Medico = consulta.getId_medico();
            Medico medico = medicoDao.buscarPorId(id_Medico);
    ...
    private void btnconfimarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnconfimarActionPerformed
        ...
        Paciente paciente = pacientecontroller.cadastrar(nomepaciente, cpf, telefone);
       if (paciente != null && paciente.getId() > 0) {
        JOptionPane.showMessageDialog(this, "Paciente cadastrado com ID: " + paciente.getId());
}
        ...
        MedicoController medicocontroller = new MedicoController();
         ...
         Medico medico = medicocontroller.cadastrar(nomemedico, especialidade, crm);
       if (medico!= null && medico.getId() > 0) {
        JOptionPane.showMessageDialog(this, "Medico cadastrado com ID: " + medico.getId());
}
        
  int id_paciente = Integer.parseInt(txtidpaciente.getText().trim());
  int id_medico = Integer.parseInt(txtidmedico.getText().trim());

LocalDate dateConsulta;
try {
    DateTimeFormatter dateFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
    dateConsulta = LocalDate.parse(txtdata.getText().trim(), dateFormatter);
} catch (DateTimeParseException e) {
    JOptionPane.showMessageDialog(null, "Data inválida. Use o formato yyyy-MM-dd (ex: 2024-03-14).");
    return;
}
...
try {
    ...
    horaConsulta = LocalTime.parse(txthora.getText().trim(), timeFormatter);
} catch (DateTimeParseException e) {
    JOptionPane.showMessageDialog(null, "Hora inválida. Use o formato HH:mm (ex: 14:30).");
    return;
}
...
consulta.setData(dateConsulta);
consulta.setHora(horaConsulta);
consulta.setObservacao(observacao);


ConsultaController consultaController = new ConsultaController();
boolean sucesso = consultaController.cadastrar(consulta);
